...
var filter = function filter() {
  ...
  var typeFilter = function typeFilter(imagesFilter) {
    ...
    if (imagesFilter.length) {
      imagesFilter.forEach(function (image) {
  ...
  itemsLi.forEach(function (li) {
    li.addEventListener('click', function (e) {
      var target = e.target;
      var imagesFilter = wrapper.querySelectorAll(".".concat(target.classList[0]));
...
var scrolling = function scrolling(upSelector) {
  ...
  }); // Плавный скролл через requestAnimationFrame
